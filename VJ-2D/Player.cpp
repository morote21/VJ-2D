...
void Player::init(const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram)
{
	...
	sprite->setAnimationSpeed(DIE_RIGHT, 10);
	for (float i = 0; i < 16; i++) {
		sprite->addKeyframe(DIE_RIGHT, glm::vec2(i / 16.f, 3 / 8.f));
	}

	sprite->setAnimationSpeed(DIE_LEFT, 10);
	for (float i = 0; i < 16; i++) {
		sprite->addKeyframe(DIE_LEFT, glm::vec2(i / 16.f, 7 / 8.f));
	}
